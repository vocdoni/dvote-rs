...
pub fn digest_string_claim(claim: &str) -> StringResult {
...
pub fn digest_hex_claim(hex_claim: &str) -> StringResult {
...
pub fn generate_mnemonic(size: i32) -> StringResult {
    let size = match size {
...
pub fn compute_private_key(phrase: &str, hd_path: &str) -> StringResult {
...
pub fn compute_public_key(hex_private_key: &str) -> StringResult {
...
pub fn compute_address(hex_private_key: &str) -> StringResult {
    let private_key_bytes = decode_hex_string(hex_private_key)?;
...
pub fn sign_message(message: &str, hex_private_key: &str) -> StringResult {
    let private_key_bytes = decode_hex_string(hex_private_key)?;
...
pub fn recover_message_signer(hex_signature: &str, message: &str) -> StringResult {
...
pub fn is_valid_signature(hex_signature: &str, message: &str, hex_public_key: &str) -> bool {
    // skip 0x
    ...
    match recover_message_signer(hex_signature, message) {
...
pub fn checksum_ethereum_address(hex_address: &str) -> StringResult {
...
mod tests {
    ...
    fn should_verify_valid_signatures() {
        ...
        // 1
        let signature = sign_message(message, &priv_key).unwrap();
        let valid = is_valid_signature(&signature, &message, &pub_key);
        ...
        let valid = is_valid_signature(&signature, "another-different-message", &pub_key);
        assert_eq!(valid, false, "Signature should be invalid");

        let signature = sign_message("message-1234", &priv_key).unwrap();
        let valid = is_valid_signature(&signature, "random-message-here", &pub_key);
        assert_eq!(valid, false, "Signature should be invalid");
