...
mod tests {
    ...
    fn should_hash_hex_claims() {
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "KqHhHze1Yongjk1Nk0UG9DvigLoXK34iROGW1TYwRZM=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "ErpbeU3kxV5cdebvHgnB/7bouSIpEtU4J5Z9ztNPSOQ=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "D1tHJqoTw6v3UqSIr4gOsHtncjnUULIYYsE88+2Ix6I=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "L0JsP7WLH+o4NVrHRtjG5P8Lc6ZQ1BCmsWQHNXLJjfU=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "ADDJIb38VKLrW8pdVaO7Xzw5RNfTlOnUFv5PqRd5XC4=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "HCulKTAZSGYENWbu53+OYnbZlvFc+TVJC09Dhm9pJDM=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "DyGx/+scdfkY8huBwOhVMYjpQxBxubFMIMZiwM0k3Mo=");
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "D50hRRqCG9nkYlNj17ZHgBTCqR6Q1Ja+H98TlNhcLlU=");
    }
    ...
    fn should_return_32_byte_hashes() {
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "AEqwLqSkyHJ+WHatCuM6xdFRaZ2oIfSjhyCAJao6k7g=");
        let len = base64::decode(hex_hash)
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "ABFnLa7ZGDakME6T8kD7euvLtfYjO1Q5OZ3F4u1PDPc=");
        let len = base64::decode(hex_hash)
            .expect("The hash is not a valid base64")
            .len();
        assert_eq!(len, 32);
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "AEsqOOrJwnOBleUz09Y2AdcsvWWy1QHWj6UG+FDBLok=");
        let len = base64::decode(hex_hash)
            .expect("The hash is not a valid base64")
            .len();
        assert_eq!(len, 32);
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "ACv4XLyQcQurGszQ1kEPd0iN4+LeLgnxxO2TSittLAg=");
        let len = base64::decode(hex_hash)
            .expect("The hash is not a valid base64")
            .len();
        assert_eq!(len, 32);
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "AH5y9pHZUL6uaOWNUq5nLmv4vITm8rW0Pa/PoaIa8N8=");
        let len = base64::decode(hex_hash)
            .expect("The hash is not a valid base64")
            .len();
        assert_eq!(len, 32);
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "AFK5bctJ4PTkQbbWV18DiECOC5CJVa6M4FU1+I4gIWI=");
        let len = base64::decode(hex_hash)
            .expect("The hash is not a valid base64")
            .len();
        assert_eq!(len, 32);
        ...
        let hex_hash = digest_hex_claim_inner(hex_claim);
        assert_eq!(hex_hash, "AL8QgTeqVomz6LHh/DCBADIFlVgLCLu1rYZMkCmw+lA=");
        let len = base64::decode(hex_hash)
            .expect("The hash is not a valid base64")
            .len();
        assert_eq!(len, 32);
